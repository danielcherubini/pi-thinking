...
describe("buildMutedMarkdownTheme: fallback anchor when thinkingText ANSI is unparseable", () => {
	...
	test("skips code-block bg when thinkingText ANSI is unparseable", () => {
		// Can't derive a harmonizing bg without an anchor — output should be
		// the plain dimmed lines, no bg open/close escapes.
		const stub = makeStubTheme({ thinkingText: "" });
		const t = buildMutedMarkdownTheme(stub);
		const out = t.highlightCode!("x = 1", "js");
...
describe("buildMutedMarkdownTheme: code-block card background", () => {
	test("wraps every highlighted line in bg open/close ANSI", () => {
		const t = buildMutedMarkdownTheme(makeStubTheme());
		const out = t.highlightCode!("a\nbb\nccc", "js");
		expect(out.length).toBeGreaterThan(0);
		const bgOpenRe = /^\x1b\[48;2;\d{1,3};\d{1,3};\d{1,3}m/;
		for (const line of out) {
			expect(bgOpenRe.test(line)).toBe(true);
			expect(line.endsWith("\x1b[49m")).toBe(true);
		}
	});

	test("pads every line to the block's max visible width", () => {
		const t = buildMutedMarkdownTheme(makeStubTheme());
		const out = t.highlightCode!("a\nbbbb\ncc", "js");
		// Strip ANSI; compare raw visible widths.
		const widths = out.map((l) => {
			const stripped = l.replace(/\x1b\[[0-9;]*m/g, "");
			return stripped.length;
		});
		// All lines should share the same visible width (padded to the max).
		const first = widths[0];
		for (const w of widths) expect(w).toBe(first);
		expect(first).toBeGreaterThanOrEqual(4); // at least "bbbb"
	});

	test("derived bg color has low lightness (dark card)", () => {
		const t = buildMutedMarkdownTheme(makeStubTheme());
		const out = t.highlightCode!("x", "js");
		const m = /^\x1b\[48;2;(\d+);(\d+);(\d+)m/.exec(out[0]);
		expect(m).not.toBeNull();
		const rgb = { r: +m![1], g: +m![2], b: +m![3] };
		const l = rgbToHsl(rgb).l;
		// Default target lightness is 0.10
		expect(l).toBeLessThanOrEqual(0.15);
	});

	test("opts.codeBlockBg=false disables the card", () => {
		const t = buildMutedMarkdownTheme(makeStubTheme(), { codeBlockBg: false });
		const out = t.highlightCode!("x", "js");
		for (const line of out) {
			expect(line).not.toContain("\x1b[48;2;");
			expect(line).not.toContain("\x1b[49m");
	...
	test("opts.codeBlockBgLightness is honored", () => {
		const t = buildMutedMarkdownTheme(makeStubTheme(), {
			codeBlockBgLightness: 0.05,
		});
		const out = t.highlightCode!("x", "js");
		const m = /^\x1b\[48;2;(\d+);(\d+);(\d+)m/.exec(out[0]);
		expect(m).not.toBeNull();
		const rgb = { r: +m![1], g: +m![2], b: +m![3] };
		const l = rgbToHsl(rgb).l;
		expect(l).toBeLessThanOrEqual(0.08);
	});
});
